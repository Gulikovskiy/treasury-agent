...
export async function discoverErc20s(chainId: ChainId, address: Address): Promise<TokenDiscovery> {
  ...
  do {
    ...
    const result = await rpc<AlchemyTokenBalancePage>(chainId, "alchemy_getTokenBalances", [
      address,
      "erc20",
      options,
    ]);
...
export async function historicalPrice(body: HistoricalPriceRequest): Promise<unknown> {
  const response = await fetch(`https://api.g.alchemy.com/prices/v1/${apiKey}/tokens/historical`, {
    method: "POST",
    headers: { "content-type": "application/json" },
