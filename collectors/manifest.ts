...
function accountingPolicy(): FixtureManifest["accountingPolicy"] {
  return {
    nav: {
      ...
      excludedFromNav: ["aave_v3_a_token", "aave_v3_variable_debt_token"],
      rules: [
    ...
    flows: {
      ...
      transferIdentityKey:
        "Alchemy uniqueId where alchemyTransfers is canonical; chainId + providerEventIndex where erc20Transfers is canonical",
      transferEventSource: {
        1: "alchemyTransfers",
      ...
      classificationOrder: ["internal_transfer", "bridge", "defi_movement", "external_flow"],
      rules: [
...
function inputTrustPolicy(): FixtureManifest["inputTrustPolicy"] {
  return {
    ...
    tokenDisplayMetadata:
      "untrusted; retained in raw balances only; excluded from normalized views",
...
const organizationalAttribution: FixtureManifest["walletOwnership"]["organizationalAttribution"] = {
  ...
  statement:
    "Aave governance identifies this address as the AFC spender on Ethereum, Avalanche, Arbitrum, and Base.",
  limitation:
    "The governance record establishes organizational attribution; pinned Safe reads separately verify technical control configuration, not legal beneficial ownership.",
};
...
export async function collectManifest(): Promise<FixtureManifest> {
  ...
  const requested =
    process.env.SNAPSHOT_TIMESTAMP ?? new Date(Date.now() - 5 * 60_000).toISOString();
  ...
  const configuredWallets = new Set(
    Object.values(TREASURY_WALLETS)
      .flat()
  ...
  for (const wallet of configuredWallets) {
    const evidence = Object.values(CHAINS)
      .filter((chain) =>
        TREASURY_WALLETS[chain.id].some((candidate) => candidate.toLowerCase() === wallet),
      )
      .map((chain) => ({
        chainId: chain.id,
        evidence: ownershipChains[chain.id][wallet],
      }));
    const configurations = evidence.map(({ evidence: item }) =>
      item?.verified
        ? `${[...(item.owners ?? [])]
    ...
    crossChainConsistency[wallet] = {
      ...
      ownerSetAndThresholdMatch:
        configurations.every(Boolean) && new Set(configurations).size === 1,
    };
  }
