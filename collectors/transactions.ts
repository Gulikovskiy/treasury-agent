...
import { blockscoutErc20Transfers, blockscoutNormalTransactions } from "../lib/blockscout.js";
import type {
  AccountHistoryFixtureResult,
  Erc20Transfer,
  NormalTransaction,
} from "../lib/account-history.js";
import { routescanErc20Transfers, routescanNormalTransactions } from "../lib/routescan.js";
      ...
      (transfer) =>
        ({
          ...
          ...(transfer.metadata && typeof transfer.metadata === "object"
            ? {
                metadata: pickFields(transfer.metadata as Record<string, unknown>, [
                  "blockTimestamp",
                ]),
              }
            : {}),
...
export async function collectTransactions(): Promise<TransactionsFixture> {
  ...
  for (const chain of Object.values(CHAINS)) {
    ...
    for (const wallet of TREASURY_WALLETS[chain.id]) {
      ...
      const normal = safeAccountHistory<NormalTransaction>(normalRaw, NORMAL_TRANSACTION_FIELDS);
      ...
      const erc20 = safeAccountHistory<Erc20Transfer>(erc20Raw, ERC20_TRANSFER_FIELDS, true);
