...
import { eventsInRange, generateGroundTruth } from "./ground-truth.js";
...
describe("ground-truth oracle checks", () => {
  it("independently values Avalanche USDC.e from raw units and snapshot price", async () => {
    ...
    const price = priceSeries.reduce((nearest, point) =>
      ...
      Math.abs(Date.parse(nearest.timestamp) - snapshot)
        ? point
        : nearest,
    );
    const position = (
      nav.positions as Array<{
        chainId: number;
        contractAddress: string;
        source: string;
        valueUsd: string;
      ...
      (candidate) =>
        candidate.chainId === 43114 &&
        candidate.contractAddress === address &&
        candidate.source === "spot",
    )!;
    ...
    const cents =
      (BigInt(balance.tokenBalanceDecimal) * priceUnits * 100n + denominator / 2n) / denominator;
  ...
  describe("generated report cross-foots", () => {
    ...
    it("cross-foots chain allocations to NAV", () => {
      const total = parseUsdCents(nav.total);
      expect(sum(Object.values(nav.byChain).map(({ value }) => parseUsdCents(value)))).toBe(total);
    });

    it("cross-foots asset allocations to NAV", () => {
      const total = parseUsdCents(nav.total);
      expect(sum(Object.values(nav.byAsset).map(({ value }) => parseUsdCents(value)))).toBe(total);
    });

    it("cross-foots signed asset percentages to 100%", () => {
      expect(
        sum(
          Object.values(nav.byAsset).map(({ percentOfNav }) =>
            parsePercentBasisPoints(percentOfNav),
          ),
        ),
      ).toBe(10_000n);
    });
    ...
    it("cross-foots Ethereum collateral components to enabled collateral", () => {
      expect(sum(Object.values(leverage.ethereumCollateralByAsset).map(parseUsdCents))).toBe(
