...
export async function getPositions(input: { chainId?: ChainId } = {}): Promise<{
  ...
  const selected = input.chainId == null
    ? fixture.positions
    : fixture.positions.filter((position) => position.chainId === input.chainId)
  return {
    positions: selected.map((position) => Object.fromEntries(
...
export async function getPrices(input: { assetIds: string[] }): Promise<{
  ...
  return {
    ...
    prices: [...new Set(input.assetIds)].map((assetId) => priceForAsset(
      fixture,
      assetId,
    )),
  }
...
function priceForAsset(fixture: PricesFixture, assetId: string): PriceResult {
  ...
  const series = identifier === 'native'
    ? chain?.native
    : chain?.tokens?.[identifier]
  if (!series) return { assetId, error: 'not_found' }
  const observations = series.data?.filter((point): point is { value: string; timestamp: string } =>
  ...
  const nearest = observations.reduce((best, point) =>
    Math.abs(Date.parse(point.timestamp) - snapshot)
      < Math.abs(Date.parse(best.timestamp) - snapshot) ? point : best)
  return {
    assetId,
